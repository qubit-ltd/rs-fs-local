...
use qubit_fs::{
    CreateDirectoryOptions, DeleteOptions, FileSystemId, FsErrorKind, Path, PersistFailureState,
    PersistOptions, TempDirectoryOptions, TempFileOptions, TempResourceState, WriteOptions,
};
...
fn test_host_temp_resources_persist_and_cleanup() {
    ...
    let file_system = LocalFileSystems::host().expect("host filesystem must be created");
