...
fn test_local_provider_modes_select_expected_authority() {
    ...
    assert_eq!(
        "local-host",
        host_resolution.file_system().properties().info().id().as_str()
    );

    let root = tempfile::tempdir().expect("provider root must be created");
    let rooted_id = FileSystemId::new("provider-mode-root").expect("rooted provider identity must be valid");
    let rooted_registry = FileSystemRegistry::default();
    rooted_registry
        .register(
            LocalFileSystemProvider::rooted(rooted_id.clone(), root.path(), LocalResourcePolicy::unbounded())
    ...
    assert_eq!(&rooted_id, rooted_resolution.file_system().properties().info().id());
}
...
fn test_rooted_local_providers_can_use_distinct_registry_descriptors() {
    ...
    let first_descriptor = ProviderDescriptor::new(ProviderId::new("rooted-first").expect("provider ID must be valid"))
        .with_aliases(["rooted-first-file"])
        .expect("provider alias must be valid");
    let second_descriptor =
        ProviderDescriptor::new(ProviderId::new("rooted-second").expect("provider ID must be valid"))
    ...
    let first = FileSystemConfig::new(ConnectionUri::parse("file:///inside").expect("URI must parse"))
        .with_selection(ProviderSelection::named("rooted-first").expect("selection must parse"));
    let second = FileSystemConfig::new(ConnectionUri::parse("file:///inside").expect("URI must parse"))
        .with_selection(ProviderSelection::named("rooted-second").expect("selection must parse"));
    assert_eq!(
        "rooted-first",
        registry
...
fn test_rooted_provider_identity_is_scoped_beyond_canonical_uri() {
    let first_root = tempfile::tempdir().expect("first root must be created");
    let second_root = tempfile::tempdir().expect("second root must be created");
    let first_id = FileSystemId::new("identity-first-root").expect("first filesystem ID must be valid");
    let second_id = FileSystemId::new("identity-second-root").expect("second filesystem ID must be valid");
    let first_descriptor =
        ProviderDescriptor::new(ProviderId::new("identity-first").expect("first provider ID must be valid"))
            .with_aliases(["identity-first-file"])
            .expect("first provider alias must be valid");
    let second_descriptor =
        ProviderDescriptor::new(ProviderId::new("identity-second").expect("second provider ID must be valid"))
    ...
    let uri = ConnectionUri::parse("file:///identity-scope.txt").expect("identity-scope URI must parse");
    let first = registry
        .resolve_config(
            &FileSystemConfig::new(uri.clone())
                .with_selection(ProviderSelection::named("identity-first").expect("first selection must parse")),
        )
        .expect("first rooted provider must resolve");
    let second = registry
        .resolve_config(
            &FileSystemConfig::new(uri)
                .with_selection(ProviderSelection::named("identity-second").expect("second selection must parse")),
        )
    ...
    assert_eq!("identity-first", first.file_system().properties().info().provider_id(),);
    assert_eq!(
        "identity-second",
...
fn test_rooted_provider_errors_retain_descriptor_identity() {
    let root = tempfile::tempdir().expect("provider root must be created");
    let descriptor = ProviderDescriptor::new(ProviderId::new("rooted-errors").expect("provider ID must be valid"))
    ...
    let resolution = registry
        .resolve_config(
            &FileSystemConfig::new(ConnectionUri::parse("file:///missing").expect("URI must parse"))
                .with_selection(ProviderSelection::named("rooted-errors").expect("selection must parse")),
        )
