...
impl RootedFixture {
    ...
    fn native_path(&self, path: &Path) -> FixtureResult<PathBuf> {
        match fs::symlink_metadata(self.root.path()) {
            ...
            Ok(_) => {
                return Err(FixtureError::new(
                    "example fixture root is not a directory",
                ));
            }
            Err(error) => {
                return Err(FixtureError::with_source(
            ...
            .components()
        {
            ...
            match fs::symlink_metadata(&current) {
                Ok(metadata) if metadata.file_type().is_symlink() => {
                    return Err(FixtureError::new(
                        "example path contains a symbolic link",
                    ));
                }
    ...
    fn teardown_entries(&self) -> FixtureResult<()> {
        ...
        for entry in fs::read_dir(self.root.path()).map_err(|error| {
            FixtureError::with_source("example root cleanup read failed", error)
        })? {
            let entry = entry.map_err(|error| {
                FixtureError::with_source(
                    "example root cleanup entry failed",
                    error,
                )
            })?;
