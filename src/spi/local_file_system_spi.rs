...
impl FileSystemSpi for LocalFileSystemSpi {
    ...
    fn open_reader(&self, request: OpenReaderRequest<'_>) -> FsResult<OpenedReader> {
        ...
        let reader = super::local_range_reader::LocalRangeReader::new(
            reader,
    ...
    fn create_temp_file(&self, request: CreateTempFileRequest) -> FsResult<OpenedTempFile> {
        ...
        let path = self.logical_path(
            public_path.as_deref().unwrap_or(value.path()),
            FsOperation::CreateTemp,
        )?;
...
mod tests {
    ...
    fn test_request_list_behavior_is_not_inherited_from_native_defaults() {
        ...
        assert!(
            entries
                .next_entry()
                .expect("end of nonrecursive listing")
                .is_none(),
    ...
    fn test_request_delete_behavior_is_not_inherited_from_native_defaults() {
        ...
        assert!(
            filesystem
                .delete_directory(
                    &Path::parse("/absent").expect("missing path"),
        ...
        assert!(
            filesystem
                .delete_directory(
                    &Path::parse("/nested").expect("nested path"),
                    DeleteOptions::default()
                )
                .is_err()
        );
